...
execute unless score *compare.equal _six matches 1 run data modify storage six:_ eval[-1].out.a append from storage six:_ eval[-1].in.a[-1]
execute if score *compare.equal _six matches 1 run function six:_/impl/set/compare/remove_b with storage six:_ eval[-1].v.matches[-1]
